...
Phase 0 + Phase 1 MVP。一套基于本地知识库 + 阿里云通义千问的客服智能问答系统，多租户、多行业从架构第一行代码就支持。
...
└──────────────┘               │                        │
                ...
                ▼                                       ▼
         ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌─────────────┐
         │ Postgres │  │  Qdrant  │  │  MinIO   │  │ DashScope   │
         │ (元数据) │  │ (向量库) │  │ (文件)   │  │ (Qwen API)  │
         └──────────┘  └──────────┘  └──────────┘  └─────────────┘
```
...
cd backend
cp .env.example .env
# 编辑 .env，填入 DASHSCOPE_API_KEY 和 LLM_MODEL
...
LLM_MODEL=qwen-plus     # 或者你确认的具体 qwen 模型 ID
```
